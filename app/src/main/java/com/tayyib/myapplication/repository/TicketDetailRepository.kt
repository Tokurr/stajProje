...
class TicketDetailRepository @Inject constructor(private val api: Api) {

    suspend fun getTicketDetail(token: String, ticketId: String): Flow<Resource<TicketDetailResponse>> {

        return flow{
         emit(Resource.Loading())

         val response = try{
            api.getTicketDetails(token, ticketId)
