...
class MyTicketRepository @Inject constructor(private val api: Api){
    suspend fun getMyTicket(token:String,id:String): Flow<Resource<TicketResponse>>
    {
        return flow {
            emit(Resource.Loading())
            val response = try{ api.getMyTickets(token,id) }
